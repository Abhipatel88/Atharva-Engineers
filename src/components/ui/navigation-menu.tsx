	...
	return (
		<div className="absolute top-full left-1/2 -translate-x-1/2 isolate z-50 flex justify-center">
			<NavigationMenuPrimitive.Viewport
				...
				className={cn(
					'origin-top-center bg-white supports-[backdrop-filter]:bg-white/95 text-zinc-950 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden rounded-md border border-slate-200 shadow-lg backdrop-blur-xl md:w-[var(--radix-navigation-menu-viewport-width)]',
					className,
				)}
				{...props}
			/>
...
function NavGridCard({
	...
	return (
		<NavigationMenuPrimitive.Link asChild>
			<Link href={link.href} className={cn("block", className)} {...props}>
				<GridCard className="h-full bg-white border-slate-100 hover:bg-slate-50">
					{link.icon && (
						<link.icon className="text-zinc-600 relative size-5" />
					)}
...
function NavSmallItem({
	...
	return (
		<NavigationMenuLink
			...
			{...props}
		>
			<Link href={item.href} className={cn('group relative flex h-max flex-row items-center gap-x-3 px-2 py-2 hover:bg-slate-50 rounded-md', className)}>
				{item.icon && <item.icon className="text-zinc-500" />}
				<p className="text-sm text-zinc-900">{item.title}</p>
				<div className="relative ml-auto flex h-full w-4 items-center">
					<ArrowRightIcon className="size-4 -translate-x-2 opacity-0 transition-all group-hover:translate-x-0 group-hover:opacity-100 text-zinc-400" />
				</div>
			</Link>
		</NavigationMenuLink>
	);
...
function NavLargeItem({
	...
	return (
		<NavigationMenuLink
			...
			{...props}
		>
			<Link href={link.href} className={cn('bg-white hover:bg-slate-50 group relative flex flex-col justify-center border border-slate-100 p-0', className)}>
				<div className="flex items-center justify-between px-5 py-4 w-full">
					<div className="space-y-1">
						<span className="text-sm leading-none font-medium text-zinc-900">{link.title}</span>
						{link.description && (
